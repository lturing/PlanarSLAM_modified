...
int ORBmatcher::MatchORBPoints(Frame & CurrentFrame, const Frame &LastFrame)
{
    ...
    descriptor1= CurrentFrame.mDescriptors;
    descriptor2=LastFrame.mDescriptors;
    //Step5
    ...
    //cout <<"ORB  matching"<<min_dist<<endl;
    for(int i=0;i<matches.size();i++)
    {
        //cout <<"ORB  matching 1"<<endl;
        if(matches[i].distance<max(2*min_dist,DIST_THRESHOLD))
        {
            //cout <<"ORB  matching 2"<<endl;
            good_matches.push_back(matches[i]);
            //cout <<"ORB  matching 3"<<endl;
            //注意这两个Idx是不一样的
...
//            points2.push_back(keyPoint2[matches[i].trainIdx].pt);
    ...
    int NPair = good_matches.size();
    //cout <<"×××××××××××××point match:"<<NPair<<endl;
    for (int i=0; i<NPair;i++)
    {
        // good matches in
        MapPoint * pMP = LastFrame.mvpMapPoints[good_matches[i].trainIdx];
        if(pMP)
        {
            if(!LastFrame.mvbOutlier[i])
            {
                CurrentFrame.mvpMapPoints[good_matches[i].queryIdx]=pMP;
            }
        }
    }
